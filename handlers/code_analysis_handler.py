...
class CodeAnalysisToolHandler(BaseHandler):
            ...
            if total_files > 0:
                for lang in sorted_stats:
                    sorted_stats[lang] = {
                        ...
                        "percentage": round((sorted_stats[lang] / total_files) * 100, 2)
                    }
            
            # Format the results
            results = ["Language distribution in the repository:"]
            for lang, stats in sorted_stats.items():
                results.append(f"- {lang}: {stats['count']} files ({stats['percentage']}%)")
            
            if not results[1:]:
                return ToolExecution(content="No recognized programming languages found in the repository.")
    ...
    async def _find_todos(self) -> ToolExecution:
        ...
        try:
            ...
            if todo_list:
                ...
                return ToolExecution(content="No TODO comments found in the repository.")
        except Exception as e:
    ...
    async def _analyze_complexity(self, file_path: str) -> ToolExecution:
        ...
        if ext != ".py":
            return ToolExecution(content=f"Error: Complexity analysis is currently only supported for Python files")
        
        try:
            # Check if the radon module is available
            try:
                subprocess.run(["pip", "show", "radon"], capture_output=True, check=True)
            except subprocess.CalledProcessError:
                return ToolExecution(content="Error: The 'radon' package is not installed. Unable to analyze complexity.")
            
            # Run complexity analysis using radon
            result = subprocess.run(
                ["radon", "cc", "-s", full_path],
                capture_output=True,
                text=True
            )
            ...
            if result.stdout:
                ...
                return ToolExecution(content=f"No complexity analysis results for {file_path}. The file might be empty or have very simple functions.")
        except Exception as e:
    ...
    async def _search_code(self, query: str) -> ToolExecution:
        ...
        try:
            ...
            if result.stdout:
                ...
                return ToolExecution(content=f"No matches found for '{query}' in the repository")
        except Exception as e:
    ...
    async def _get_dependencies(self) -> ToolExecution:
        ...
        try:
            ...
            if package_json_files:
                dependencies["javascript"] = {"files": [], "packages": {"dependencies": [], "devDependencies": []}}
                        ...
                        try:
                            ...
                            if "dependencies" in pkg_data:
                                for dep, version in pkg_data["dependencies"].items():
                                    dependencies["javascript"]["packages"]["dependencies"].append(f"{dep}@{version}")
                            
                            if "devDependencies" in pkg_data:
                                for dep, version in pkg_data["devDependencies"].items():
                                    dependencies["javascript"]["packages"]["devDependencies"].append(f"{dep}@{version}")
                        except json.JSONDecodeError:
                            pass
            
            # Format the results
            if not dependencies:
                return ToolExecution(content="No dependency files found in the repository")
            ...
            if "python" in dependencies:
                ...
                if dependencies["python"]["packages"]:
                    ...
                    if len(dependencies["python"]["packages"]) > 20:
                        results.append(f"  - ... and {len(dependencies['python']['packages']) - 20} more")
            ...
            if "javascript" in dependencies:
                ...
                if dependencies["javascript"]["packages"]["dependencies"]:
                    ...
                    for pkg in dependencies["javascript"]["packages"]["dependencies"][:20]:
                        results.append(f"  - {pkg}")
                    
                    if len(dependencies["javascript"]["packages"]["dependencies"]) > 20:
                        results.append(f"  - ... and {len(dependencies['javascript']['packages']['dependencies']) - 20} more")
                
                if dependencies["javascript"]["packages"]["devDependencies"]:
                    results.append("  Dev Dependencies:")
                    for pkg in dependencies["javascript"]["packages"]["devDependencies"][:20]:
                        results.append(f"  - {pkg}")
                    
                    if len(dependencies["javascript"]["packages"]["devDependencies"]) > 20:
                        results.append(f"  - ... and {len(dependencies['javascript']['packages']['devDependencies']) - 20} more")
