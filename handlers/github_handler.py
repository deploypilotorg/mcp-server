    ...
    async def execute(self, params: Dict[str, Any]) -> ToolExecution:
        ...
        try:
            ...
            return ToolExecution(
                content=f"Successfully cloned repository: {repo_url} to {self.repo_path}"
            )
...
class GitHubListFilesToolHandler(BaseHandler):
    ...
    async def execute(self, params: Dict[str, Any]) -> ToolExecution:
        ...
        if not self.clone_handler.repo_path or not os.path.exists(
            self.clone_handler.repo_path
        ):
