...
class UIGeneratorToolHandler(BaseHandler):
    ...
    async def execute(self, params: Dict[str, Any]) -> ToolExecution:
        ...
        if not self.repo_path:
            return ToolExecution(
                content="Error: No repository is currently cloned. Please clone a repository first."
            )
    ...
    async def _scan_apps(self) -> ToolExecution:
        ...
        try:
            ...
            if not app_files:
                return ToolExecution(
                    content="No potential application entry points found in the repository."
                )
            ...
            for file_path in app_files:
                ...
                try:
                    with open(full_path, "r", encoding="utf-8", errors="ignore") as f:
                        content = f.read(
                            2000
                        )  # Read the first 2000 characters for analysis
            ...
            if not app_info:
                return ToolExecution(
                    content="No recognizable applications found in the repository."
                )
    ...
    async def _generate_ui(self, app_path: str) -> ToolExecution:
        ...
        if not os.path.exists(full_path):
            return ToolExecution(
                content=f"Error: Application path '{app_path}' does not exist"
            )
        ...
        try:
            ...
            if app_path.endswith(".py"):
                ...
                return ToolExecution(
                    content=f"Unsupported application type for {app_path}. Currently supporting .py, .js, and .html files."
                )
    ...
    async def _run_python_app(self, app_path: str) -> ToolExecution:
        ...
        if "streamlit" in content.lower():
            ...
            cmd = [
                ...
                "-m",
                "uvicorn",
                os.path.basename(full_path).replace(".py", ":app"),
                "--port",
                str(port),
            ]
            app_url = f"http://localhost:{port}"
        ...
        try:
            if has_requirements:
                ...
                if install_process.returncode != 0:
                    return ToolExecution(
                        content=f"Error installing dependencies:\n{install_process.stderr}"
                    )
            ...
            if process.poll() is not None:
                ...
                return ToolExecution(
                    content=f"Application failed to start:\n\nSTDOUT:\n{stdout}\n\nSTDERR:\n{stderr}"
                )
    ...
    async def _run_js_app(self, app_path: str) -> ToolExecution:
        ...
        try:
            ...
            if has_package_json:
                ...
                with open(
                    package_json_path, "r", encoding="utf-8", errors="ignore"
                ) as f:
                    package_data = json.load(f)

                # Install dependencies
                install_process = subprocess.run(
                    ["npm", "install"], capture_output=True, text=True, cwd=app_dir
                )

                if install_process.returncode != 0:
                    return ToolExecution(
                        content=f"Error installing npm dependencies:\n{install_process.stderr}"
                    )
                ...
                if "scripts" in package_data and "start" in package_data["scripts"]:
                    ...
                    cmd = ["node", full_path]
                    app_url = (
                        "No web interface directly available for this Node.js script"
                    )
            else:
...
To stop the application, use the 'stop_ui' action with the session ID.
            ...
            return ToolExecution(
                content=f"Error running JavaScript application: {str(e)}"
            )
